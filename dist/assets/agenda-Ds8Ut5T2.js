import{s as w,m as h,g as v,e as s,b as p,h as y,a as b,f as x,u as E,k as C}from"./index-BbYPTjK5.js";import{c as m}from"./card-CijOLgPO.js";import{o as H}from"./modal-sZK26qrI.js";import{s as f,t as r,v as L,g as B}from"./form-DLgGHwbw.js";const u="2026-07-20",T=24*60*60*1e3;let I=[],a={date:u,medicoId:"",consultorio:"",estado:"",selectedCitaId:null};async function O(e,o={},t={}){w("Agenda / Citas","Gestione y programe las citas de sus pacientes"),a={date:u,medicoId:"",consultorio:"",estado:"",selectedCitaId:null},e.innerHTML=`
    <div class="view">
  ...
  `,M(),c(),document.getElementById("btn-nueva-cita").addEventListener("click",D),document.getElementById("agenda-date-input").addEventListener("change",i=>{a.date=i.target.value,a.selectedCitaId=null,c()}),document.getElementById("btn-prev-day").addEventListener("click",()=>$(-1)),document.getElementById("btn-next-day").addEventListener("click",()=>$(1)),document.getElementById("btn-today").addEventListener("click",()=>{a.date=u,document.getElementById("agenda-date-input").value=u,c()}),["filter-medico","filter-consultorio","filter-estado"].forEach(i=>{document.getElementById(i).addEventListener("change",n=>{const d=i==="filter-medico"?"medicoId":i==="filter-consultorio"?"consultorio":"estado";a[d]=n.target.value,c()})}),t.pacienteId&&(a.pacienteFiltro=t.pacienteId)}function $(e){const o=new Date(`${a.date}T00:00:00`);o.setTime(o.getTime()+e*T),a.date=o.toISOString().slice(0,10),document.getElementById("agenda-date-input").value=a.date,a.selectedCitaId=null,c()}function M(){const e=h(),o=v("medicos"),t=document.getElementById("filter-medico");t.innerHTML='<option value="">Todos los doctores</option>'+o.map(d=>`<option value="${d.id}">${s(d.nombre)}</option>`).join("");const i=document.getElementById("filter-consultorio");i.innerHTML='<option value="">Todos los consultorios</option>'+e.consultorios.map(d=>`<option value="${s(d)}">${s(d)}</option>`).join("");const n=document.getElementById("filter-estado");n.innerHTML='<option value="">Todos los estados</option>'+e.estadosCita.map(d=>`<option value="${d}">${p(d)}</option>`).join("")}function F(){return v("citas").filter(e=>e.fecha===a.date).filter(e=>!a.medicoId||e.medicoId===a.medicoId).filter(e=>!a.consultorio||e.consultorioId===a.consultorio).filter(e=>!a.estado||e.estado===a.estado).filter(e=>!a.pacienteFiltro||e.pacienteId===a.pacienteFiltro).sort((e,o)=>e.horaInicio.localeCompare(o.horaInicio))}function c(){var o,t;const e=F();(!a.selectedCitaId||!e.some(i=>i.id===a.selectedCitaId))&&(a.selectedCitaId=((o=e.find(i=>i.pacienteId))==null?void 0:o.id)||((t=e[0])==null?void 0:t.id)||null),k(e),S(e.find(i=>i.id===a.selectedCitaId)),q(e)}function k(e){const o=document.getElementById("agenda-list-container");o.innerHTML=m({title:`Agenda · ${x(a.date)}`,bodyHtml:e.length?`<div class="stack" id="agenda-list">${e.map(t=>{const i=t.pacienteId?y("pacientes",t.pacienteId):null,n=i?`${i.nombre} ${i.apellidos}`:t.motivo;return`
    ...
    `});const n=document.getElementById("btn-cancelar-cita");n&&n.addEventListener("click",async()=>{await E("citas",e.id,{estado:"cancelada"}),c()})}function q(e){const o=document.getElementById("agenda-summary-container"),t=e.reduce((i,n)=>(i[n.estado]=(i[n.estado]||0)+1,i),{});o.innerHTML=m({title:"Resumen del día",bodyHtml:`
  ...
  `;H({title:"Nueva cita",bodyHtml:i,footerHtml:`
      <button type="button" class="btn btn-secondary" id="modal-cancel">Cancelar</button>
      <button type="button" class="btn btn-primary" id="modal-save">Guardar cita</button>
    `,onMount:(n,d)=>{n.querySelector("#modal-cancel").addEventListener("click",d),n.querySelector("#modal-save").addEventListener("click",async()=>{const g=n.querySelector("#form-nueva-cita");if(!L(g))return;const l=B(g);await C("citas",{pacienteId:l.pacienteId,medicoId:l.medicoId,consultorioId:l.consultorioId,fecha:l.fecha,horaInicio:l.horaInicio,horaFin:l.horaFin,motivo:l.motivo,estado:"confirmada",notas:"",recordatorios:[]}),d(),a.date=l.fecha,document.getElementById("agenda-date-input").value=l.fecha,c()})}})}function G(){I.forEach(e=>e()),I=[]}export{O as mount,G as unmount};
