import{h as b,s as E,m as w,i as S,e as l,j as F,n as y,k as T,o as $}from"./index-BbYPTjK5.js";import{t as n,a as r,v as M,g as P}from"./form-DLgGHwbw.js";let u=[],m=null;async function z(s,o={}){const a=b("pacientes",o.id);if(!a){s.innerHTML='<div class="empty-state">Paciente no encontrado.</div>';return}E("Consulta médica",`${a.nombre} ${a.apellidos}`);const i=w(),c=b("medicos","MED-0001"),d=Date.now();s.innerHTML=`
    <div class="view">
...
Dieta hiposódica.`})}
  ...
  `,D(),B(d),document.getElementById("btn-cancelar-consulta").addEventListener("click",()=>{y(`#/historia-clinica/${a.id}`)}),document.getElementById("form-consulta").addEventListener("submit",async t=>{var h;t.preventDefault();const v=t.target;if(!M(v))return;const e=P(v),x=[...document.getElementById("f-diagnosticos").selectedOptions].map(p=>{const[I,C]=p.value.split("|");return{cie10:I,descripcion:C,tipo:"definitivo"}}),g=parseFloat(e.peso)||null,f=parseFloat(e.talla)||null;await T("consultas",{pacienteId:a.id,medicoId:(c==null?void 0:c.id)||"MED-0001",fecha:new Date().toISOString(),tipo:"seguimiento",motivoConsulta:e.motivoConsulta,padecimientoActual:e.padecimientoActual||"",sintomas:e.sintomas?e.sintomas.split(",").map(p=>p.trim()).filter(Boolean):[],signosVitales:{ta:e.ta||"",fc:parseInt(e.fc,10)||null,fr:parseInt(e.fr,10)||null,temp:parseFloat(e.temp)||null,spo2:parseInt(e.spo2,10)||null,peso:g,talla:f,imc:$(g,f)},exploracionFisica:e.exploracionFisica||"",antecedentes:{heredofamiliares:e.heredofamiliares||"",personalesPatologicos:e.personalesPatologicos||"",personalesNoPatologicos:e.personalesNoPatologicos||""},diagnosticos:x,planTerapeutico:e.planTerapeutico?e.planTerapeutico.split(`
`).map(p=>p.trim()).filter(Boolean):[],notas:e.notas||"",duracion:((h=document.getElementById("consulta-timer"))==null?void 0:h.textContent)||"00:00:00",estado:"completada"}),y(`#/historia-clinica/${a.id}`)})}function D(){const s=document.querySelector('[name="peso"]'),o=document.querySelector('[name="talla"]'),a=document.getElementById("consulta-imc-display");function i(){const c=parseFloat(s.value),d=parseFloat(o.value),t=$(c,d);a.textContent=t?`${t} kg/m²`:"—"}s.addEventListener("input",i),o.addEventListener("input",i),u.push(()=>{s.removeEventListener("input",i),o.removeEventListener("input",i)})}function B(s){const o=document.getElementById("consulta-timer");function a(){const i=Math.floor((Date.now()-s)/1e3),c=String(Math.floor(i/3600)).padStart(2,"0"),d=String(Math.floor(i%3600/60)).padStart(2,"0"),t=String(i%60).padStart(2,"0");o&&(o.textContent=`${c}:${d}:${t}`)}a(),m=setInterval(a,1e3)}function N(){u.forEach(s=>s()),u=[],m&&(clearInterval(m),m=null)}export{z as mount,N as unmount};
